...
## Next Up - Phase 9: Browser Notifications
- [ ] `notifications` table + model + migration
- [ ] Browser push notifications
- [ ] Dashboard notification banner
- [ ] Mark notifications as read
- [ ] `GET /notifications`
- [ ] Wire notification creation into `run_daily_pipeline` (app/scheduler/jobs.py) after the ranking refresh step
...
- Phase 8's "Update notifications" task was not implemented - there's no notifications system yet (Phase 9's explicit scope). `run_daily_pipeline` (app/scheduler/jobs.py) already fetches and re-ranks daily; once Phase 9 adds notification creation logic, it's a small addition to call it right after the ranking-refresh step, using that step's already-computed top matches.

...
## v1.0.0 - Phase 11: Final Refactor
V1 complete - all 11 phases implemented, tested, and documented. This phase made no behavior changes; it's a hardening/cleanup pass.
- **Formatting/linting**: added `backend/pyproject.toml` configuring Black and Ruff (previously specified in `docs/12_CODING_GUIDELINES.md` but never actually enforced). Applied Black across the entire backend (55 files, purely cosmetic - reran the full suite after, still 124/124). Fixed the Ruff findings that were real: added `strict=True` to a `zip()` call in the matching engine (three lists guaranteed same length by construction; now fails loudly instead of silently truncating if that guarantee is ever broken by a future change). FastAPI's required `Depends()`/`Query()`-in-defaults pattern and the `str`+`Enum` mixin pattern (used for DB storage and Pydantic validation) are explicitly configured as intentional, not silently ignored.
- **Dead code / consistency audit**: searched for TODO/FIXME/HACK markers (none found), cross-checked every route module is registered in `main.py` (8/8), cross-checked every frontend HTML page's links and script tags resolve to real files (no broken references, no orphaned JS), and verified the full Alembic migration chain (`0001`-`0007`) produces a schema that exactly matches the SQLAlchemy models' schema (checked programmatically, not by eye).
- **Documentation**: fixed real staleness in `docs/03_TECH_STACK.md` (listed `requests`; the project actually uses `httpx`, plus was missing `beautifulsoup4`) and `docs/13_TESTING_STRATEGY.md` (updated "Manual: Dashboard, Notifications, Scheduler" to reflect what actually ended up with full automated coverage vs. what's still genuinely manual - a browser visual pass). Marked all phases complete in `docs/11_DEVELOPMENT_ROADMAP.md`.
- **README**: full rewrite - accurate feature list (nothing aspirational), architecture diagram, complete environment variable reference table, API endpoint summary, testing instructions, and a dedicated "Known Limitations & Design Decisions" section consolidating the honest tradeoffs noted throughout `docs/TODO.md` across all 10 previous phases (no true push notifications, per-source-only job dedup, expiry-detection is a heuristic, rule-based resume parsing, etc.) so they're visible without having to read the full phase-by-phase history.
- Bumped the app version to `1.0.0` (`app/main.py` and this changelog) to mark V1 as complete.
